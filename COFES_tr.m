function [eta_RT,rho_E_L,t_generation,erro]= COFES_tr(varargin)
...
%-------------------- Pump isoentropic efficiency ------------------------%
    if any(strcmp('eta_p',varargin))
        ...
        eta_c = varargin{find(strcmp('eta_p',varargin))+1};
    end
...
% Properties at the evaporator 1 outlet / turbine inlet
...
% State at condenser 1 outlet / pump inlet
    P_4(1) = P_LPT(1); %!!!
    x_4(1) = 0;
...
%             error('The fluid at the HPT must be either liquid or a liquid-vapour mixture.')
%         end
    % State at Evaporator 1 outlet / Turbine inlet
    ...
    % State at condenser 1 outlet / LPT inlet
        P_4(i) = P_LPT(i); %!!!
        x_4(i) = 0;
...
%     TSDiag=plotTS(fluid,'Iso_P',[P_HPT(1),P_LPT(1),P_HPT(end),P_LPT(end)]);
    ...
    plotTS(fluid,'Process','Isobaric',P_1(1),s_1(1),s_2(1),'plotColor','k');
    plotTS(fluid,'Process','Expansion',P_2(1),h_2(1),P_3(1),eta_t,'plotColor','k');
    handle_disch(2) = plotTS(fluid,'Process','Isobaric',P_3(1),s_3(1),s_4(1),'plotColor','k');
    
    plotTS(fluid,'Process','Isobaric',P_1(n_steps_disch),s_1(n_steps_disch),s_2(n_steps_disch),'plotColor',[0.7 0.7 0.7]);
    plotTS(fluid,'Process','Expansion',P_1(n_steps_disch),h_2(n_steps_disch),P_3(n_steps_disch),eta_t,'plotColor',[0.7 0.7 0.7]);
    handle_disch(3) = plotTS(fluid,'Process','Isobaric',P_3(n_steps_disch),s_3(n_steps_disch),s_4(n_steps_disch),'plotColor',[0.7 0.7 0.7]);
    grid on;
...
% State at LPT outlet / Evaporator 2 inlet
...
% State at evaporator 2 outlet / compressor inlet
P_6(1) = P_LPT(n_steps_disch) - DeltaP_Ev; % INCLUIR VERIFICAÇÃO DE ESTADO LIQUIDO SATURADO OU SUBRESFRIADO
x_6(1) = 1;
T_6(1) = CoolProp.PropsSI('T','P',P_6(1),'Q',x_6(1),fluid);
h_6(1) = CoolProp.PropsSI('H','P',P_6(1),'Q',x_6(1),fluid);
s_6(1) = CoolProp.PropsSI('S','P',P_6(1),'Q',x_6(1),fluid);

% State at compressor outlet / condenser inlet
% P_6(1) = P_HPT(1); % VERIFICAR
P_7(1) = P_HPT(n_steps_disch);
h_c(1) = CoolProp.PropsSI('H','P',P_7(1),'S',s_6(1),fluid);
h_7(1) = compressor('h_in',h_6(1),'h_s',h_c(1),'eta_c',eta_c);
T_7(1) = CoolProp.PropsSI('T','P',P_6(1),'H',h_6(1),fluid);
s_7(1) = CoolProp.PropsSI('S','P',P_6(1),'H',h_6(1),fluid);    

% State at condenser outlet / HPT inlet
P_8(1) = P_7(1) - DeltaP_Con;
x_8(1) = 0;
T_8(1) = CoolProp.PropsSI('T','P',P_8(1),'Q',x_8(1),fluid);
h_8(1) = CoolProp.PropsSI('H','P',P_8(1),'Q',x_8(1),fluid);
s_8(1) = CoolProp.PropsSI('S','P',P_8(1),'Q',x_8(1),fluid); 

% Energy transfers during the first time step
w_comp(1) = h_7(1) - h_6(1);
q_Ev_2(1) = h_6(1) - h_5(1);
...
switch Heat_source
    case 'HPs' % Systems with heat pump to provide the heat at the Heater
        ...
        w_c_HP2(1) = q_Ev_2(1)/COP_HP2(1);
        q_L_HP2(1) = q_Ev_2(1) - w_c_HP2(1);
        w_c(1) = w_comp(1)+w_c_HP2(1);
%         w_c(1) = w_p(1);
    case 'Free'
        w_c(1) = w_comp(1);
end
...
dE_comp(1) = m_charg(1)*w_comp(1)*dt;
dQ_he(1) = m_charg(1)*q_Ev_2(1)*dt;
...
%         end
    ...
    % State at evaporator 2 outlet / compressor inlet
        P_6(i) = P_LPT(n_steps_disch+i); % INCLUIR VERIFICAÇÃO DE ESTADO LIQUIDO SATURADO OU SUBRESFRIADO
        x_6(i) = 1;
        T_6(i) = CoolProp.PropsSI('T','P',P_6(i),'Q',x_6(i),fluid);
        h_6(i) = CoolProp.PropsSI('H','P',P_6(i),'Q',x_6(i),fluid);
        s_6(i) = CoolProp.PropsSI('S','P',P_6(i),'Q',x_6(i),fluid);
    
    % State at compressor outlet / condenser 2 inlet    
    %     P_6(i) = P_HPT(1); % VERIFICAR
        P_7(i) = P_HPT(n_steps_disch+i);
        h_c(i) = CoolProp.PropsSI('H','P',P_7(i),'S',s_6(i),fluid);
        h_7(i) = compressor('h_in',h_6(i),'h_s',h_c(i),'eta_c',eta_c);
        T_7(i) = CoolProp.PropsSI('T','P',P_7(i),'H',h_7(i),fluid);
        s_7(i) = CoolProp.PropsSI('S','P',P_7(i),'H',h_7(i),fluid);    

    % State at condenser 2 outlet / HPT inlet
        P_8(i) = P_7(i) - DeltaP_Con;
        x_8(i) = 0;
        T_8(i) = CoolProp.PropsSI('T','P',P_8(i),'Q',x_8(i),fluid);
        h_8(i) = CoolProp.PropsSI('H','P',P_8(i),'Q',x_8(i),fluid);
        s_8(i) = CoolProp.PropsSI('S','P',P_8(i),'Q',x_8(i),fluid);

    % Energy transfers during i'th time step
        w_comp(i) = h_7(i) - h_6(i);
        q_Ev_2(i) = h_6(i) - h_5(i);
        ...
        switch Heat_source
            case 'HPs' % Systems with heat pump to provide the heat at the Heater
                ...
                w_c_HP2(i) = q_Ev_2(i)/COP_HP2(i);
                q_L_HP2(i) = q_Ev_2(i) - w_c_HP2(i);
                w_c(i) = w_comp(i)+w_c_HP2(i);
    %             w_c(i) = w_p(i);
            case 'Free'
                w_c(i) = w_comp(i);
        end
        ...
        m_charg(i) = W_c/w_c(i);
        dE_comp(i) = m_charg(i)*w_comp(i)*dt;
        dQ_he(i) = m_charg(i)*q_Ev_2(i)*dt;
...
%     TSDiag=plotTS(fluid,'Iso_P',[P_HPT(n+1),P_LPT(n+1),P_HPT(end),P_LPT(end)]);
    ...
    plotTS(fluid,'Process','Isobaric',P_5(1),s_5(1),s_6(1),'plotColor','k');
    plotTS(fluid,'Process','Pump_comp',P_6(1),h_6(1),P_7(1),eta_c,'plotColor','k');
    handle_charg(2) = plotTS(fluid,'Process','Isobaric',P_7(1),s_7(1),s_8(1),'plotColor','k');
    
    plotTS(fluid,'Process','Isobaric',P_5(end),s_5(end),s_6(end),'plotColor','k');
    plotTS(fluid,'Process','Pump_comp',P_6(end),h_6(end),P_7(end),eta_c,'plotColor',[0.7 0.7 0.7]);
    handle_charg(3) = plotTS(fluid,'Process','Isobaric',P_7(end),s_7(end),s_8(end),'plotColor',[0.7 0.7 0.7]);
    grid on;
...
%=================== Full-cycle performance parameters ===================%
    ...
    E_comp = sum(dE_comp); % Total work consumed in the pump, in J
