function [eta_RT,eta_I,eta_II,q_in,w_net]=COFES_st(varargin)
% ORCES_ST ORCES steady-state operation model
%   ORCES_st()
...
%------------ PRESSURE DROP IN THE EVAPORATOR/SUPERHEATERS ---------------%
...
%------------------- PRESSURE DROP IN THE CONDENSERS ---------------------%
...
%------------------ Compressor isoentropic efficiency --------------------%
    if any(strcmp('eta_c',varargin))
        if ~isnumeric(varargin{find(strcmp('eta_c',varargin))+1})
            error('eta_c must be a number, not a %s.',class(eta_c))
        end
        eta_c = varargin{find(strcmp('eta_c',varargin))+1};
    end
...
% State at the evaporator 1 outlet / turbine inlet
    P(2) = P(1) - DeltaP_Ev;
    T(2) = T_2;
...
% State at evaporator 2 outlet
    P(6) = P_LPT - DeltaP_Ev;
    x(6) = 1;
    T(6) = CoolProp.PropsSI('T','P',P(6),'Q',x(6),fluid);
    h(6) = CoolProp.PropsSI('H','P',P(6),'Q',x(6),fluid);
    s(6) = CoolProp.PropsSI('S','P',P(6),'Q',x(6),fluid);
   
% State at compressor outlet / condenser 2 inlet
    P(7) = P_HPT; %!!!
    h_c = CoolProp.PropsSI('H','P',P(7),'S',s(6),fluid);
    h(7) = compressor('h_in',h(6),'h_s',h_c,'eta_c',eta_c);
    T(7) = CoolProp.PropsSI('T','P',P(7),'H',h(7),fluid);
    s(7) = CoolProp.PropsSI('S','P',P(7),'H',h(7),fluid);

% State at condenser 2 outlet / HPT inlet
    P(8) = P_HPT; %!!!
    x(8) = 0;
    T(8) = CoolProp.PropsSI('T','P',P(8),'Q',x(8),fluid);
    h(8) = CoolProp.PropsSI('H','P',P(8),'Q',x(8),fluid);
    s(8) = CoolProp.PropsSI('S','P',P(8),'Q',x(8),fluid);
    ...
    switch Heat_source
        case 'HPs' % System with heat pump to provide heat at the evaporator/superheater
            ...
            if T(2)>T_HPT % If there is superheating
                ...
                q_SH = h(2) - h_g_Ev;
                q_lg = q_Ev_1 - q_SH;
                T_H1 = (q_lg/q_Ev_1)*T_HPT + (q_SH/q_Ev_1)*(T_HPT+T(2))/2;
            ...
            w_c_HP1 = q_Ev_1/COP_HP1; % Work for reversible heat pump
            q_L_HP1 = q_Ev_1 - w_c_HP1;      % Heat extracted from heat source
...
%             T_H2 = T(7);
            ...
            w_c_HP2 = q_Ev_2/COP_HP2;
            q_L_HP2 = q_Ev_2 - w_c_HP2;
            
            q_in = q_L_HP1 + q_L_HP2;
            w_net = w_t - w_c - w_c_HP1 - w_c_HP2;
            w_generated = w_t - w_c_HP1;
%             w_g(1) = w_t(1);
            w_consumed = w_c + w_c_HP2;
        case 'Free'
            q_in = q_Ev_1 + q_Ev_2;
            w_net = w_t - w_c;
            w_generated = w_t;
...
%---------------------------- Results display ----------------------------%
    if plotBool
        ...
        plotTS(fluid,'Process','Isobaric',P(1),s(1),s(2)) % Evaporator 1
        plotTS(fluid,'Process','Isobaric',P(3),s(3),s(4)) % Condenser 1
        
        plotTS(fluid,'Process','Isobaric',P(5),s(5),s(6)) % Evaporator 2
        plotTS(fluid,'Process','Isobaric',P(7),s(7),s(8)) % Condenser 2
        
        plotTS(fluid,'Process','Expansion',P(2),h(2),P(3),eta_t) % Expander
        plotTS(fluid,'Process','Pump_comp',P(6),h(6),P(7),eta_c) % Condenser
